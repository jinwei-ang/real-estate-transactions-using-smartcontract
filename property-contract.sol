...
contract LandContract {
    ...
    enum PropertyStatus {
        ...
        Sold,
        Bought
    } // Propertystatus of Pending -> Available for sale, Sold -> Property no longer belong to this Owner, Bought -> Property not up for sale but belong to this Owner
    ...
    uint256[] public PropertyPending;
    ...
    function getNumberOfLands() public view returns (uint256) {
    ...
    function addLand(
        ...
        public
        isAdmin //Add property to a property owner
    {
        ...
        emit Add(
            ...
            _PropertyAddress,
            propertystatus
        ); //trigger event to output property info added
    }

    function getContainsLandId(address _propertyOwner, uint256 _PropertyId)
        internal
        view
        returns (
            bool // Boolean output to check if a particular property exist based on propertyOwner address and PropertyId
        )
    {
        for (
            uint256 i = 0;
            i < Ownedlands[_propertyOwner].length;
            i++ // using a for loop to loop through _propertyOwner array to check if _PropertyId matches with PropertyId
        ) {
            if (
                Ownedlands[_propertyOwner][i].PropertyId == _PropertyId
    ...
    function buyland(
        ...
        public
        isAdmin // Used for transferring property ownership from _propertyOwner to _propertyBuyer
    {
        require(_propertyOwner != _propertyBuyer); // require that the _propertyOwner and _propertyBuyer are of different address

        uint256 i = 0; // initialise variable i to be 0
        while (
            Ownedlands[_propertyOwner][i].PropertyId != _PropertyId // while loop to search for the index of _propertyOwner array's PropertyId that matches with the _PropertyId
        ) {
            i++;
        }

        require(
            Ownedlands[_propertyOwner][i].propertystatus ==
                PropertyStatus.Pending
        ); // once index is found, use that to confirm that the propertystatus is Pending, otherwise not able to transact
        ...
        emit Add(
            _propertyBuyer,
            _PropertyId,
            Ownedlands[_propertyOwner][i].PropertyValue,
            Ownedlands[_propertyOwner][i].PropertyAddress,
            PropertyStatus.Bought
        ); //trigger event to output property info added
    }

    function getallpending(address _propertyOwner)
        public
        returns (
            uint256[] memory // return an array of all the PropertyId that have propertystatus Pending
        )
    {
        ...
        for (
            uint256 i = 0;
            i < Ownedlands[_propertyOwner].length;
            i++ // for loop to loop through Ownedlands array based on _OwnerAddress
        ) {
            // if given land ID is indeed in owner's collection
            if (
                Ownedlands[_propertyOwner][i].propertystatus ==
    ...
    function getpropertyinfo(
        address _propertyOwner,
        uint256 _PropertyId // Obtain property info based on _propertyOwner address and _PropertyId
    ) public {
        ...
        emit propertyinfo(
            ...
            Ownedlands[_propertyOwner][i].PropertyValue,
            Ownedlands[_propertyOwner][i].PropertyAddress
        ); // trigger event propertyinfo to output property info
    }

    function editproperty(
        address _propertyOwner,
        uint256 _PropertyId,
        uint256 _NewPropertyValue
    )
        public
        isAdmin // change the PropertyValue to _NewPropertyValue of a given PropertyId based on _propertyOwner and _PropertyId input
    {
        ...
        require(
            Ownedlands[_propertyOwner][i].propertystatus != PropertyStatus.Sold
        ); // if the propertystatus is Sold it means the property no longer belong to _propertyOwner, do not proceed
        Ownedlands[_propertyOwner][i].PropertyValue = _NewPropertyValue; // replace old PropertyValue with _NewPropertyValue
    }

    function putupforsale(address _propertyOwner, uint256 _PropertyId)
        public
        isAdmin // change the propertystatus to Pending of a given PropertyId based on _propertyOwner and _PropertyId input
    {
        require(getContainsLandId(_propertyOwner, _PropertyId) == true); // making sure _PropertyId belongs to _propertyOwner otherwise do not proceed
